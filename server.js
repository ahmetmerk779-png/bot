...
const guiPlugin = require('mineflayer-gui'); // Güncel import
...
function createBot(config) {
  ...
  bot.once('spawn', () => {
    ...
    bot.loadPlugin(guiPlugin); // Güncel plugin yükleme
...
// --- Socket.IO İletişimi ve Yeni Özellikler ---
io.on('connection', (socket) => {
  ...
  // *** YENİ VE GÜNCELLENMİŞ GUI İŞLEMİ (clickWindow + mineflayer-gui) ***
  socket.on('select-game', async (data) => {
    if (!bot) return sendLog('Bot bağlı değil!', 'error');
    const { itemName, gameName } = data;
    sendLog(`🔍 GUI üzerinden "${itemName}" kullanılarak "${gameName}" seçilmeye çalışılıyor...`, 'info');

    try {
      // 1. Eşyayı envanterde bul ve donat
      ...
      // 2. Eşyaya sağ tıklayarak GUI'yi aç
      bot.activateItem();
      sendLog(`🖱️ "${itemName}" eşyasına sağ tıklandı. GUI açılması bekleniyor...`, 'info');

      // 3. GUI'nin açılmasını bekle (mineflayer-gui veya clickWindow için)
      ...
      sendLog(`🪟 GUI açıldı: ${window.title} (Toplam slot: ${window.slots.length})`, 'success');

      // 4. Oyun modunu GUI'de bul (displayName ile)
      let targetSlot = -1;
      for (let i = 0; i < window.slots.length; i++) {
        const slot = window.slots[i];
        if (slot && slot.displayName && slot.displayName.includes(gameName)) {
          targetSlot = i;
          break;
        }
      }
      ...
      // 5. Seçilen oyun moduna tıkla (Hem mineflayer-gui hem de clickWindow ile)
      // Yöntem A: mineflayer-gui ile (eğer yüklüyse)
      if (bot.gui && bot.gui.Query) {
        const comparator = (arg, it) => it.displayName && it.displayName.includes(arg);
        const result = await bot.gui.Query()
          .Window(comparator)
          .Click(gameName, "left", 1)
          .close()
          .run();
        if (result) sendLog(`🎮 "${gameName}" GUI üzerinden başarıyla seçildi!`, 'success');
        else sendLog(`⚠️ GUI tıklama başarısız oldu.`, 'error');
      } 
      // Yöntem B: Yerleşik clickWindow ile (her durumda çalışır)
      else {
        // 0: sol tık, 0: normal tıklama modu
        bot.clickWindow(targetSlot, 0, 0, (err) => {
          if (err) return sendLog(`❌ clickWindow hatası: ${err.message}`, 'error');
          sendLog(`🎮 "${gameName}" clickWindow ile başarıyla seçildi!`, 'success');
          // GUI'yi kapat
      ...
      sendLog(`⚠️ GUI işlemi sırasında hata: ${err.message}`, 'error');
    }
  });
});
