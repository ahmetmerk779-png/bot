...
function createBot(config) {
  ...
  bot.on('health', () => {
    ...
    if (autoEatEnabled && bot.food < 18) {
      const foodItem = bot.inventory.slots.find(s => s && (s.name.includes('cooked') || s.name.includes('bread')));
      if (foodItem) {
...
io.on('connection', (socket) => {
  ...
  socket.on('connect-bot', (c) => { botConfig = c; createBot(c); });
...
// --- Web Arayüzü (Mobil Uyumlu, Tüm Özellikler) ---
